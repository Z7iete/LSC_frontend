...
export const useRecordsStore = defineStore("registros", () => {
  ...
  const totalDestacados = computed(() => {
    return registrosActivos.value.filter(
      (item) => item.destacado == 1 || item.destacado === true,
    ).length;
  });
    ...
    try {
      ...
      if (!respuesta.ok) {
        const detalle = resultado.errores
          ...
          : resultado.mensaje || "No se pudo agregar el registro";
  ...
  async function desactivarRegistro(id) {
    ...
    try {
      const respuesta = await fetch(
        `${URL_BACKEND}/registros/${id}/desactivar`,
        {
          method: "PUT",
        },
      );
      ...
      if (!respuesta.ok) {
        const detalle = resultado.errores
          ? resultado.errores.join(", ")
          : resultado.mensaje || "No se pudo desactivar el registro";
        throw new Error(detalle);
      }
